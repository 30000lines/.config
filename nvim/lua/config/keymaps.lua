...
map("n", "<leader>ag", "<cmd>lua require('fzf-lua').live_grep({fzf_opts = {['--layout'] = 'reverse-list'}})<CR>", { desc = "fzf-lua current buffer" })
...
-- map("n", "<leader>tf", Util.telescope("files"), { desc = "Telescope Find Files (root dir)" })
-- map("n", "<leader>tg", Util.telescope("live_grep"), { desc = "Telescope Grep (root dir)" })
-- map("n", "<leader>tb", "<cmd>Telescope current_buffer_fuzzy_find<cr>", { desc = "Telescope Buffer" })
